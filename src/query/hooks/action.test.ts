...
import { useActions, useAggregateActions, useMemberActions } from './action.js';
...
describe('Action Hooks', () => {
  ...
  describe('useActions', () => {
    ...
    it(`Sample size = 0 does not fetch`, async () => {
      const hook = () => useActions({ ...args, requestedSampleSize: 0 });
      const endpoints = [{ route, response }];
    ...
    it(`enabled=false does not fetch`, async () => {
      const hook = () => useActions(args, { enabled: false });
      const endpoints = [{ route, response }];
      const { data, isFetched } = await mockHook({
        endpoints,
        hook,
  ...
  describe('useAggregateActions', () => {
    ...
    it(`Receive aggregate actions for item id`, async () => {
      const hook = () => useAggregateActions(itemId, args);
      const endpoints = [{ route, response }];
      const { data } = await mockHook({ endpoints, hook, wrapper });
      expect(data).toEqual(response);
  ...
  describe('useMemberActions', () => {
    ...
    it(`Receive actions for current member`, async () => {
      const hook = () => useMemberActions(args);
      const endpoints = [{ route, response }];
      const { data } = await mockHook({ endpoints, hook, wrapper });
      expect(data).toEqual(response);
